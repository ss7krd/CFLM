...
vals = pd.read_csv('accuracy_trainingTime_bar_real_vgg.csv')
# vals = pd.read_csv('accuracy_trainingTime_data_each_comp_real.csv')

cflmd = vals['cflmd'].tolist()
rects1 = ax.bar(ind[0], cflmd[0], width, color=[(1,0.7,0)],edgecolor='black',label="CFLMD")

bfl = vals['bfl'].tolist()
rects2 = ax.bar(ind[0]+width, bfl[0], width, color='pink',edgecolor='black',label="FedCPF")

afl = vals['afl'].tolist()
ax.bar(ind[0]+width*2, afl[0], width, color = 'cyan', edgecolor='black', label="LDF")

cfl = vals['cfl'].tolist()
rects3 = ax.bar(ind[0]+width*3, cfl[0], width, color='darkgrey',edgecolor='black',label="CEFL")

# new_fl = vals['FL'].tolist()
# rects4 = ax.bar(ind[0]+width*3, new_fl[0], width, color='purple', edgecolor='black',hatch='/o', label="BFL1")

lgc = vals['lgc'].tolist()
rects1 = ax.bar(ind[0]+width*4, lgc[0], width, color='yellow', edgecolor='black', label="LGC")
...
rects3 = ax.bar(ind[0]+width*5, cflmd_m[0], width, color='tomato',edgecolor='black',label="CFLMD/M")

cflmd_cs = vals['cflmd_cs'].tolist()
rects4 = ax.bar(ind[0]+width*6, cflmd_cs[0], width, color='deepskyblue',edgecolor='black',label="CFLMD/CS")

cflmd_fd = vals['cflmd_fd'].tolist()
rects1 = ax.bar(ind[0]+width*7, cflmd_fd[0], width, color='lime',edgecolor='black',label="CFLMD/FD")

cflmd_de = vals['cflmd_de'].tolist()
rects2 = ax.bar(ind[0]+width*8, cflmd_de[0], width,color = 'beige', edgecolor = 'black', label = "CFLMD/DE")
...
cflmd = vals['cflmd'].tolist()
rects1 = ax2.bar(ind[1], cflmd[1], width, color=[(1,0.7,0)],edgecolor='black',label="TrustMe")

bfl = vals['bfl'].tolist()
rects2 = ax2.bar(ind[1]+width, bfl[1], width, color='pink',edgecolor='black',label="BFL")

afl = vals['afl'].tolist()
ax2.bar(ind[1]+width*2, afl[1], width, color = 'cyan', edgecolor='black', label="afl")

cfl = vals['cfl'].tolist()
rects3 = ax2.bar(ind[1]+width*3, cfl[1], width, color='darkgrey',edgecolor='black',label="TOM")

# new_fl = vals['FL'].tolist()
# rects4 = ax2.bar(ind[1]+width*3, new_fl[1], width, color='purple', edgecolor='black',hatch='/o', label="BFL1")

lgc = vals['lgc'].tolist()
rects1 = ax2.bar(ind[1]+width*4, lgc[1], width, color='yellow', edgecolor='black', label="lgc")

# cflmd_wo_group = vals['cflmd_wo_group'].tolist()
# rects2 = ax2.bar(ind[1]+width*5, cflmd_wo_group[1], width, color='crimson',edgecolor='black',hatch="|*")

cflmd_m = vals['cflmd_m'].tolist()
rects3 = ax2.bar(ind[1]+width*5, cflmd_m[1], width, color='tomato',edgecolor='black')

cflmd_cs = vals['cflmd_cs'].tolist()
rects4 = ax2.bar(ind[1]+width*6, cflmd_cs[1], width, color='deepskyblue',edgecolor='black')

cflmd_fd = vals['cflmd_fd'].tolist()
rects1 = ax2.bar(ind[1]+width*7, cflmd_fd[1], width, color='lime',edgecolor='black')

cflmd_de = vals['cflmd_de'].tolist()
rects2 = ax2.bar(ind[1]+width*8, cflmd_de[1], width,color = 'beige', edgecolor = 'black')
